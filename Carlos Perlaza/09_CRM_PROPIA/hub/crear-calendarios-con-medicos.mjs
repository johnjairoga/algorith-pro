...
const CALENDARIOS = [
  {
    ...
    medicos_asignados: ['medico_001', 'medico_002'], // Dra. Anja, Dr. Aristides
  ...
  },
  {
    ...
    eventColor: '#7B1FA2',
    pipeline: 'Consulta Inicial',
    medicos_asignados: ['medico_001'], // Dra. Anja (especialista)
  ...
  },
  {
    ...
    medicos_asignados: ['medico_003'], // Dra. Dafne
  ...
  },
  {
    ...
    eventColor: '#388E3C',
    pipeline: 'Recurrencia',
    medicos_asignados: ['medico_002', 'medico_003'], // Dr. Aristides, Dra. Dafne
...
async function crearCalendariosConMedicos() {
  try {
    console.log('\n📅 CREANDO 8 CALENDARIOS CON MÉDICOS EN GHL...\n');
    ...
    for (let i = 0; i < CALENDARIOS.length; i++) {
      ...
      const medicosDelCalendario = medicos.filter(m =>
        cal.medicos_asignados.includes(m.id)
      );

      console.log(`[${i + 1}/${CALENDARIOS.length}] 📅 ${cal.name}`);
      console.log(`   Duración: ${cal.slotDuration} min | Pipeline: ${cal.pipeline}`);
      console.log(`   Médicos: ${medicosDelCalendario.map(m => m.nombre_completo).join(', ')}`);

      try {
        const payload = {
          isActive: true,
          locationId: LOCATION_ID,
          teamMembers: medicosDelCalendario.map((m, idx) => ({
            userId: m.id,
        ...
        results.byCalendar[cal.name] = {
          ...
          medicos: medicosDelCalendario.map(m => ({ id: m.id, nombre: m.nombre_completo }))
        };
    ...
    if (results.success.length > 0) {
      ...
      results.success.forEach(name => {
        ...
        console.log(`     Médicos: ${cal.medicos.map(m => m.nombre).join(', ')}\n`);
      });
    }
    ...
    // Guardar resultados
    const resultsPath = path.resolve('./results/fase4b-calendarios-con-medicos.json');
    fs.mkdirSync(path.dirname(resultsPath), { recursive: true });
    fs.writeFileSync(resultsPath, JSON.stringify(results, null, 2));

    console.log(`📊 Resultados guardados en: results/fase4b-calendarios-con-medicos.json\n`);

    if (results.failed.length === 0) {
      console.log('🎉 ¡TODOS LOS CALENDARIOS FUERON CREADOS CON ÉXITO!\n');
      console.log('✅ Próximo paso: Crear Servicios y vincularlos a los calendarios\n');
    }
...
crearCalendariosConMedicos().then(() => {
